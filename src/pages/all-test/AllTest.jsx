...
const AllTests = () => {
    ...
    const fetchAllTests = async (page = 0, search = '', filter = '') => {
        try {
            setLoading(true);
            setError(null);
            ...
            const response = await apiClient.post('/user-activity/getAllTest', requestBody);
    ...
    useEffect(() => {
        fetchAllTests(0, searchTerm, filterTerm);
    }, []);
    ...
    const handlePageChange = (newPage) => {
        if (newPage >= 0 && newPage < totalPages) {
            fetchAllTests(newPage, searchTerm, filterTerm);
        }
    };
            ...
            {!loading && !error && (
                <>
                    ...
                    <div className="bg-white rounded-lg shadow-md overflow-x-auto border border-[#7966F1] !m-8">
                                ...
                                {testData.length > 0 ? (
                                    testData.map((test, index) => (
                                        <tr key={test.id || index} className="border-t hover:bg-gray-50">
                                            <td className="!px-6 !py-4">{test.srNo}</td>
                                            <td className="!px-6 !py-4">{test.testName}</td>
                                            <td className="!px-6 !py-4">{test.teacherName}</td>
                                            <td className="!px-6 !py-4">{test.batch}</td>
                                            <td className="!px-6 !py-4">{test.examDuration}</td>
                                            <td className="!px-6 !py-4">{test.totalQuestions}</td>
                                            <td className="!px-6 !py-4">{test.startTime}</td>
                                            <td className="!px-6 !py-4">{test.endTime}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
                                        <td colSpan="8" className="!px-6 !py-8 text-center text-gray-500">
                                            {searchTerm || filterTerm ? 'No tests found matching your criteria' : 'No tests found'}
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                        ...
                        {testData.length > 0 && totalPages > 1 && (
