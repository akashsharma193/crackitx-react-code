...
const UserRanking = () => {
    ...
    useEffect(() => {
        const userBatchCode = localStorage.getItem('batch');
        if (userBatchCode) {
            setBatchCode(userBatchCode);
            fetchRankings(userBatchCode);
        } else {
            setLoading(false);
    ...
    const fetchRankings = useCallback(async (batch) => {
        try {
            setLoading(true);
            ...
            if (response.data.success && response.data.data) {
                ...
                if (sortedData.length === 0) {
                    toast.info('No ranking data found');
                }
            } else {
                            ...
                            <div className="flex items-center justify-between flex-wrap gap-4">
                                <div>
                                    ...
                                    <div className="text-gray-600 mt-2">
                                        ...
                                        <span className="!ml-4">
                                            Batch: <span className="font-semibold text-[#7966F1]">{batchCode}</span>
                                        </span>
                                    </div>
                                </div>
