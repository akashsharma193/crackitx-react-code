      ...
      <div className="bg-white rounded-2xl shadow-xl !px-6 !py-8 max-w-md w-full mx-4 border border-gray-200">
        <div className="text-center">
          ...
          <p className="text-sm text-green-500 font-medium !mb-6">
            <span className="text-black font-semibold">Note:</span> You can go
            to Active Exams to edit the test.
          </p>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-black/30">
      <div className="bg-white rounded-2xl shadow-xl !px-6 !py-8 max-w-md w-full mx-4 border border-gray-200">
        <div className="text-center">
          ...
          <p className="text-gray-800 !mb-2">
            We are generating the question list, please wait.
          </p>
        ...
        <div className="space-y-8">
          ...
          <div>
            ...
            <div className="relative">
              <select
                ...
                onChange={(e) =>
                  handleAIInputChange("critical", e.target.value)
                }
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleAIInputChange("questionCount", e.target.value)
              }
              className="w-full !px-4 !py-3 border border-[#5E48EF] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5E48EF] focus:border-transparent bg-[#5E48EF]/5"
...
const ImportQuestionBankDialog = ({
  ...
  const handleSelectAll = () => {
    ...
    const allCurrentSelected = currentPageQuestionIds.every((id) =>
      allSelectedQuestions.includes(id),
    );
    ...
    if (allCurrentSelected) {
      ...
      setAllSelectedQuestions((prev) => {
        const newSelections = currentPageQuestionIds.filter(
          (id) => !prev.includes(id),
        );
  ...
  const handleAddSelected = () => {
    ...
    const selectedQuestionData = allSelectedQuestions
  ...
  const endIndex =
    totalElements > 0
      ...
      : 0;
  const currentPageQuestionIds = questionList.map((q) => q.id);
  const currentPageSelectedCount = currentPageQuestionIds.filter((id) =>
    allSelectedQuestions.includes(id),
  ).length;
      ...
      <div className="bg-white rounded-2xl shadow-xl !px-6 !py-8 max-w-4xl w-full mx-4 border border-gray-200 max-h-[90vh] overflow-y-auto">
        <div className="text-center !mb-6">
          ...
          <p className="text-gray-600">
            Select questions from existing question bank
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 !mb-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleBankInputChange("critical", e.target.value)
              }
              className="w-full !px-4 !py-3 border border-[#5E48EF] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5E48EF] focus:border-transparent bg-[#5E48EF]/5"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleBankInputChange("language", e.target.value)
              }
              className="w-full !px-4 !py-3 border border-[#5E48EF] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5E48EF] focus:border-transparent bg-[#5E48EF]/5"
            ...
            <div className="flex items-center justify-between !mb-4">
              ...
              <div className="flex items-center gap-4">
                <span className="text-sm text-gray-600">
                  Total Selected: {allSelectedQuestions.length}
                </span>
            ...
            <div className="max-h-96 overflow-y-auto border border-gray-200 rounded-lg !p-4 !mb-4">
              {questionList.map((question, index) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={allSelectedQuestions.includes(
                      question.id || index,
                    )}
        ...
        <div className="flex justify-center gap-4">
          ...
          {questionList.length > 0 && (
            ...
            >
              {isLoading
                ? "Adding..."
                : `Add Selected (${allSelectedQuestions.length})`}
            </button>
          )}
        </div>
      </div>
...
const QuestionCard = ({
  ...
  const [questionInputMode, setQuestionInputMode] = useState(
    question.questionInputMode || "text",
  );
  const [optionsInputMode, setOptionsInputMode] = useState(
    question.optionsInputMode || "text",
  );
                    ...
                    {option.image ? (
                      <div className="relative">
                        <img
                          src={option.image}
                          alt={`Option ${index + 1}`}
                          className="w-full max-h-32 object-contain border border-[#5E48EF] rounded-lg"
                        />
                        <button
                          onClick={() =>
                            onImageUpload(question.id, "option", null, index)
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 !mb-6">
      ...
      >
        <div className="flex items-center gap-3">
          <ChevronDown
            className={`w-5 h-5 text-[#5E48EF] transition-transform ${isOpen ? "rotate-0" : "-rotate-90"}`}
          />
          {isEditingCategory ? (
            <input
              type="text"
            ...
            <h2 className="text-[#5E48EF] text-xl font-bold">
              {editedCategoryName || "Uncategorized"}
            </h2>
          )}
          {!isEditingCategory && (
            <button
          ...
          <span className="bg-[#5E48EF] text-white text-sm !px-3 !py-1 rounded-full">
            {questions.length}
          </span>
        </div>
        <button
...
const CreateExam = () => {
  ...
  const handleImageUpload = useCallback(
    (questionId, imageType, imageData, optionIndex = null) => {
      setQuestions((prev) =>
        prev.map((q) => {
          if (q.id === questionId) {
    ...
    reader.onload = (e) => {
      try {
        ...
        jsonData.forEach((row, index) => {
          ...
          const questionType = (row["Question Type"] || "text")
            .toString()
            .toLowerCase();
          const optionType = (row["Option Type"] || "text")
            .toString()
            .toLowerCase();
          ...
          if (questionType === "text") {
            if (!row["Question"] || !row["Question"].toString().trim()) {
              toast.error(
                `Row ${rowNumber}: Question text is required when Question Type is text`,
              );
              hasErrors = true;
              return;
            }
          ...
          if (questionType === "image") {
            if (
              !row["Question Image URL"] ||
              !row["Question Image URL"].toString().trim()
            ) {
              toast.error(
                `Row ${rowNumber}: Question Image URL is required when Question Type is image`,
              );
              hasErrors = true;
              return;
            }
          ...
          if (optionType === "text") {
            ...
            if (options.some((opt) => !opt)) {
              toast.error(
                `Row ${rowNumber}: All 4 text options are required when Option Type is text`,
              );
              hasErrors = true;
              return;
            }
          ...
          if (optionType === "image") {
            ...
            if (optionImages.some((img) => !img)) {
              toast.error(
                `Row ${rowNumber}: All 4 option image URLs are required when Option Type is image`,
              );
              hasErrors = true;
              return;
            }
          ...
          if (optionType === "text") {
            ...
            if (correctAnswerIndex === -1) {
              toast.error(
                `Row ${rowNumber}: Correct answer must match one of the text options exactly`,
              );
              hasErrors = true;
              return;
            }
            ...
            if (correctAnswerNumber >= 1 && correctAnswerNumber <= 4) {
              ...
              toast.error(
                `Row ${rowNumber}: For image options, Correct Answer must be 1, 2, 3, or 4`,
              );
              hasErrors = true;
              return;
            }
          ...
          importedQuestions.push({
            ...
            questionImage:
              questionType === "image"
                ? row["Question Image URL"]?.toString().trim() || null
                : null,
            options: [
              {
                text: options[0],
                type: optionType,
                image: optionType === "image" ? optionImages[0] : null,
              },
              {
                text: options[1],
                type: optionType,
                image: optionType === "image" ? optionImages[1] : null,
              },
              {
                text: options[2],
                type: optionType,
                image: optionType === "image" ? optionImages[2] : null,
              },
              {
                text: options[3],
                type: optionType,
                image: optionType === "image" ? optionImages[3] : null,
              },
            ],
  ...
  const pollQuestionStatus = useCallback(
    async (referenceKey, currentAttempt) => {
      try {
        const response = await apiClient.get(
          `questionGenerator/statusCheckQuestionRequest/${referenceKey}`,
        );
        ...
        if (response.data && response.data.success) {
          if (
            ...
            const aiQuestions = response.data.data.map((item, index) => {
              const correctAnswerIndex = item.options
                ? item.options.findIndex((opt) => opt === item.correctAnswer)
                : 0;
            ...
            toast.success(
              `${aiQuestions.length} questions generated and added successfully!`,
            );
          } else {
  ...
  const handleAIImport = useCallback(
    async (aiData) => {
      ...
      try {
        const response = await apiClient.post(
          "questionGenerator/requestCreateQuestion",
          {
  ...
  const resolveCorrectAnswerIndex = (item, textOptions, imageOptions) => {
    if (typeof item.correctAnswer === "string") {
      const idx = textOptions.findIndex(
        (opt) => opt && opt.trim() === item.correctAnswer.trim(),
      );
      if (idx >= 0) return idx;
    }

    const parsed = parseInt(item.correctAnswer, 10);
    if (!isNaN(parsed) && parsed >= 1 && parsed <= 4) {
      return parsed - 1;
    }

    if (typeof item.correctAnswer === "number") {
      return item.correctAnswer >= 0 && item.correctAnswer < 4
        ? item.correctAnswer
        : 0;
    }
  ...
  const handleQuestionBankImport = useCallback(async (selectedQuestionData) => {
    ...
    try {
      const bankQuestions = selectedQuestionData.map((item, index) => {
        ...
        const imageOptions = Array.isArray(item.optionsImage)
          ? item.optionsImage
          : [null, null, null, null];
        ...
        if (item.question && item.questionImage) questionInputMode = "both";
        else if (item.questionImage) questionInputMode = "image";

        const hasTextOptions = textOptions.some((t) => t && t.trim());
        const hasImageOptions = imageOptions.some(Boolean);

        let optionsInputMode = "text";
        if (hasTextOptions && hasImageOptions) optionsInputMode = "both";
        else if (hasImageOptions) optionsInputMode = "image";
        ...
        return {
          ...
          questionInputMode,
          questionType: questionInputMode === "image" ? "image" : "text",

          options: [0, 1, 2, 3].map((i) => ({
            text: textOptions[i] || "",
            image: imageOptions[i] || null,
            type:
              textOptions[i] && imageOptions[i]
                ? "both"
                : imageOptions[i]
                  ? "image"
                  : "text",
      ...
      setQuestions((prev) =>
        prev.length === 1 &&
        prev[0].question === "" &&
        prev[0].options.every((o) => !o.text && !o.image)
          ? bankQuestions
          : [...prev, ...bankQuestions],
      );
      ...
      toast.success(`${bankQuestions.length} questions imported successfully`);
    } catch (err) {
      console.error(err);
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (isNaN(positiveMarks) || positiveMarks < 0) {
      toast.error(
        "Positive marks must be a valid number greater than or equal to 0",
      );
      return;
    }
    ...
    const validQuestions = questions.filter((q) => {
      ...
      if (questionInputMode === "text") {
        ...
        hasQuestion =
          (q.question && q.question.trim() !== "") || q.questionImage !== null;
      }
    ...
    if (invalidQuestions.length > 0) {
      toast.error(
        "All questions must have complete question content, all 4 options, and a correct answer selected",
      );
      return;
    }
    ...
    try {
      ...
      const questionList = validQuestions.map((q) => {
        ...
        if (optionsInputMode === "text") {
          ...
          const hasTextOptions = q.options.some(
      ...
      <div className="!p-8">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 !p-8 !mb-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleInputChange("examDuration", e.target.value)
                }
                className="w-full !px-4 !py-3 border border-[#5E48EF] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5E48EF] focus:border-transparent bg-[#5E48EF]/5"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleInputChange("positiveMarks", e.target.value)
                }
                className="w-full !px-4 !py-3 border border-[#5E48EF] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5E48EF] focus:border-transparent bg-[#5E48EF]/5"
            ...
            <div className="flex items-center gap-3">
              ...
              {!canToggleActive() && formData.startTime && (
                <span className="text-xs text-gray-400 !ml-2">
                  (Can only be toggled if start date is more than 24 hours from
                  now)
                </span>
              )}
            </div>
          ...
          <div className="flex justify-center gap-4 !mt-8 flex-wrap">
            ...
            <button
              ...
              className="bg-gradient-to-r from-[#9181F4] to-[#5038ED] text-white !px-8 !py-3 rounded-lg font-medium hover:from-[#9181F4] hover:to-[#5038ED] transition-all flex items-center gap-2 shadow-lg cursor-pointer"
            >
              Import from Excel
            ...
            <button
              ...
              className="bg-gradient-to-r from-[#FF6B6B] to-[#FF8E53] text-white !px-8 !py-3 rounded-lg font-medium hover:from-[#FF6B6B] hover:to-[#FF8E53] transition-all flex items-center gap-2 shadow-lg cursor-pointer"
            >
              Import from AI
            ...
            <button
              ...
              className="bg-gradient-to-r from-[#28A745] to-[#20C997] text-white !px-8 !py-3 rounded-lg font-medium hover:from-[#28A745] hover:to-[#20C997] transition-all flex items-center gap-2 shadow-lg cursor-pointer"
            >
              Import from Question Bank
            ...
            <button
              ...
              className="bg-gradient-to-r from-[#6C757D] to-[#495057] text-white !px-8 !py-3 rounded-lg font-medium hover:from-[#6C757D] hover:to-[#495057] transition-all flex items-center gap-2 shadow-lg cursor-pointer"
            >
              Reset
          ...
          {showQuestions && questions.length > 0 && (
            <div className="flex justify-center gap-4 !mt-6">
              <button
                ...
                className="bg-gradient-to-r from-[#4F46E5] to-[#7C3AED] text-white !px-8 !py-3 rounded-lg font-medium hover:from-[#4F46E5] hover:to-[#7C3AED] transition-all flex items-center gap-2 shadow-lg cursor-pointer"
              >
                Create New Category
        ...
        <div className="flex items-center justify-center mx-auto gap-4">
          ...
          <button
            onClick={handleSubmit}
            disabled={isLoading}
            className="bg-gradient-to-r from-[#9181F4] to-[#5038ED] text-white !px-12 !py-3 rounded-lg font-medium hover:from-[#9181F4] hover:to-[#5038ED] transition-all shadow-lg cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          >
