      ...
      <div className="bg-white rounded-2xl shadow-xl !px-6 !py-8 max-w-md w-full mx-4 border border-gray-200">
        <div className="text-center">
          ...
          <p className="text-gray-800 !mb-2">
            Do you want to disable this {type}?
          </p>
          <p className="text-sm text-red-500 font-medium !mb-6">
            <span className="text-black font-semibold">Note:</span> This action
            will deactivate the {type} account.
          </p>
...
const CreateAdmin = () => {
    ...
    try {
      if (activeTab === "admin") {
        ...
        if (response.data.success) {
          ...
          const uniqueOrgs = [
            ...new Set(content.map((admin) => admin.orgCode).filter(Boolean)),
          ];
        ...
        const response = await apiClient.post(
          "/admin-secured/getAllUserSuperAdmin",
          requestBody,
        );
        ...
        if (response.data.success) {
          ...
          const uniqueOrgs = [
            ...new Set(
              content.map((student) => student.orgCode).filter(Boolean),
            ),
          ];
          setAllOrganizations(uniqueOrgs);
        }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const endpoint =
        activeTab === "admin"
          ? "/superAdmin/enableDisableUser"
          : "/admin-secured/enableDisableUser";
      const response = await apiClient.post(endpoint, requestBody);

      if (response.data.success) {
        toast.success(
          `${activeTab === "admin" ? "Admin" : "Student"} disabled successfully!`,
        );
        ...
        throw new Error(
          response.data.message || `Failed to disable ${activeTab}`,
        );
      }
    } catch (error) {
  ...
  const hasFilters =
    activeTab === "admin"
      ...
      : debouncedSearchTerm.trim() ||
        debouncedSelectedOrg.trim() ||
        debouncedSelectedBatch.trim();
  ...
  const getFilteredBatches = () => {
    ...
    const filteredStudents = allStudentsData.filter(
      (student) => student.orgCode === selectedOrg,
    );
    const batches = [
      ...new Set(
  ...
  return (
    <div className="flex-1 !py-0 overflow-y-auto">
      ...
      {!loading && (
        <div className="!p-6">
          <table className="min-w-full text-left text-sm border border-gray-400 border-collapse">
            <thead className="bg-white text-gray-700 font-bold border-b-2 border-gray-400">
              <tr>
                <th className="!px-6 !py-4 border border-gray-400">Sr.no.</th>
                <th className="!px-6 !py-4 border border-gray-400">Name</th>
                <th className="!px-6 !py-4 border border-gray-400">Email Id</th>
                <th className="!px-6 !py-4 border border-gray-400">
                  Phone Number
                </th>
                <th className="!px-6 !py-4 border border-gray-400">
                  Organization
                ...
                {activeTab === "student" && (
                  <th className="!px-6 !py-4 border border-gray-400">Batch</th>
                )}
                <th className="!px-6 !py-4 border border-gray-400">Edit</th>
                <th className="!px-6 !py-4 border border-gray-400">Delete</th>
              </tr>
            </thead>
            ...
            <tbody>
              {currentData.length > 0 ? (
                currentData.map((item, index) => (
                  <tr key={item.id || index} className="hover:bg-gray-50">
                    ...
                    <td className="!px-6 !py-4 border border-gray-400 text-center">
                      <div className="relative group inline-block">
                        <Trash2
                          className="inline-flex items-center justify-center rounded-md !p-2 bg-red-100 text-red-500 cursor-pointer hover:text-red-700 transition-colors"
                          size={35}
                ...
                <tr>
                  <td
                    colSpan={activeTab === "admin" ? 7 : 8}
                    className="!px-6 !py-8 text-center text-gray-500 border border-gray-400"
                  >
                    {hasFilters
          ...
          {totalElements > 0 && (
            <div className="flex items-center justify-between !px-6 !py-4 border-t border-gray-400">
              <div className="text-sm text-gray-600">
                ...
                {activeTab}s
                {hasFilters && (
                  <span className="text-[#7966F1] ml-2">(filtered)</span>
                )}
              </div>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1 !px-3 !py-2 rounded-md text-sm font-medium transition ${
                      currentPage === 0
                        ...
                        : "bg-white text-[#7966F1] border border-[#7966F1] hover:bg-[#7966F1] hover:text-white"
                    }`}
                  >
                    <ChevronLeft size={16} />
                    Previous
