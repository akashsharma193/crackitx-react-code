...
const CreateAdmin = () => {
    ...
    const fetchAdmins = async (page = 0, search = '', filter = '') => {
        try {
            setLoading(true);
    ...
    useEffect(() => {
        setCurrentPage(0);
        fetchAdmins(0, debouncedSearchTerm, debouncedFilterTerm);
    }, [debouncedSearchTerm, debouncedFilterTerm]);

    // const handleViewClick = (admin) => {
    //     navigate(`/admin-details/${admin.userId}`, {
    //         state: { admin: admin }
    //     });
    // };
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                await fetchAdmins(currentPage, debouncedSearchTerm, debouncedFilterTerm);
            } else {
    ...
    const handlePageChange = (newPage) => {
        if (newPage >= 0 && newPage < totalPages && newPage !== currentPage) {
            ...
            fetchAdmins(newPage, debouncedSearchTerm, debouncedFilterTerm);
        }
    };
    ...
    const handleSearchKeyPress = (e) => {
        if (e.key === 'Enter') {
            setDebouncedSearchTerm(searchTerm);
            setCurrentPage(0);
            fetchAdmins(0, searchTerm, filterTerm);
        }
    };
                        ...
                        <tbody>
                            {admins.length > 0 ? (
                                admins.map((admin, index) => (
                                    <tr key={admin.id || index} className="border-t hover:bg-gray-50">
                                        ...
                                        <td className="!px-6 !py-4">{admin.mobile || 'N/A'}</td>
                                        <td className="!px-6 !py-4">{admin.orgCode || 'N/A'}</td>
                                ...
                                <tr>
                                    <td colSpan="8" className="!px-6 !py-8 text-center text-gray-500">
                                        {hasFilters ? 'No admins found matching your criteria' : 'No admin data available'}
                                    </td>
                                </tr>
                            )}
                        </tbody>
                    ...
                    {totalElements > 0 && (
                        <div className="flex items-center justify-between !px-6 !py-4 border-t">
                            <div className="text-sm text-gray-600">
                                Showing {startIndex} to {endIndex} of {totalElements} admins
                                {hasFilters && (
